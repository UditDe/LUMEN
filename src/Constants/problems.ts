export const problem_one : string = `
# Minimum Bit Flips to Convert Number

A **bit flip** of a number \`x\` is choosing a bit in the binary representation of \`x\` and flipping it from either \`0 → 1\` or \`1 → 0\`.

For example:  
- For \`x = 7\`, the binary representation is \`111\`.  
...
It can be shown we cannot convert \`10\` to \`7\` in fewer than 3 steps. Hence, we return **3**.
...
It can be shown we cannot convert \`3\` to \`4\` in fewer than 3 steps. Hence, we return **3**.
